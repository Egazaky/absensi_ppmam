    ...
    public function toggle(Request $request)
    {
        try {
            $this->validate($request, [
                ...
                'source' => 'nullable|string|in:scan',
            ]);
        ...
        $isScanRequest = $request->input('source') === 'scan';
            ...
            if ($isScanRequest) {
                if ($request->expectsJson() || $request->isXmlHttpRequest()) {
                    return response()->json([
                        'success' => true,
                        'duplicate' => true,
                        'message' => "Santri sudah absen pada sesi {$session}.",
                        'action' => 'sudah hadir',
                        'session' => $session,
                    ]);
                }
                ...
                return redirect()->back()
                    ->with('alert', "Santri sudah absen pada sesi {$session}.");
            }
